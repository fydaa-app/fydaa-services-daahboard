...
async function fetchPackages(
    ...
    try {
        let url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}${process.env.NEXT_PUBLIC_PACKAGES_LIST_ENDPOINT}?page=${page}&limit=${limit}`;
...
export default function PackageTablesPage() { 
    ...
    const fetchData = useCallback(async () => {
        try {
            setIsSearching(true);
            const data = await fetchPackages(currentPage, limit, searchQuery);
            setApiResponse(data);
            setError(null);
        } catch (err) {
