...
interface Stock {
  ...
  async function fetchStocks(searchQuery: string = ""): Promise<{ 
    stocks: Stock[]; 
    error: string | null;
    totalStocks?: number;
    limit?: number;
    ...
    try {
      let url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}${process.env.NEXT_PUBLIC_STOCK_ENDPOINT}`;
      ...
      return { stocks: [], error: "Error fetching data" };
    }
  }

export default function StockTablesPage() { // Renamed from UserTablesPage
  ...
  const [isSearching, setIsSearching] = useState(false);
    ...
    const fetchData = useCallback(async () => {
        try {
          const { stocks, error } = await fetchStocks(searchQuery);
          setStocks(stocks);
          setError(error);
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Stocks" /> {/* Updated title */}
            <div className="space-y-6">
                <ComponentCard title="Stock List"> {/* Updated title */}
              ...
              <StockListTable stocks={stocks} error={error} />
