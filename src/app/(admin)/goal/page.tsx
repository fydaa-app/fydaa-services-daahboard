...
async function fetchGoals(
  ...
  try {
    let url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}${process.env.NEXT_PUBLIC_GOAL_LIST_ENDPOINT}?page=${page}&limit=${limit}`;
...
export default function GoalTablesPage() { 
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const data = await fetchGoals(pageNum, limit, searchQuery);
      setApiResponse(data);
    ...
    <div>
      ...
      <div className="space-y-6">
        <ComponentCard title="Goal List"> 
          ...
          <GoalListTable goals={apiResponse.items} error={error} />
