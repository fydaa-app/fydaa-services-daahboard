...
import PackageServiceList from "@/components/tables/PackageServiceList"; 
import CreatePackageService from "@/components/form/admin-form/CreatePackageService";
...
async function fetchPackageServices(
    page: number = 1,
    limit: number = 10,
    searchQuery: string = ""
): Promise<{
    ...
    try {
        let url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}package-service?page=${page}&limit=${limit}`;
        ...
        if (!response.ok) {
            throw new Error("Failed to fetch package services");
        }
...
export default function PackageServiceTablesPage() { 
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const data = await fetchPackageServices(pageNum, limit, searchQuery);
            setApiResponse(data);
            ...
            setError("Failed to load package services");
            setApiResponse({
                items: [],
                totalItems: 0,
                totalPages: 0,
    ...
    // Callback function to refresh data after create/edit/delete operations
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Package Services" />
            <div className="space-y-6">
                <ComponentCard title="Package Service List">
                    ...
                    <PackageServiceList 
                        packageServices={apiResponse.items} 
                        error={error} 
