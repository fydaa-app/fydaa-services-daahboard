...
import RecommendedStockListTable from "@/components/tables/RecommendedStockListTable"; 
import CreateRecommendedStock from "@/components/form/admin-form/CreateRecommendedStock";
...
async function fetchRecommendedStocks(
    ...
    try {
        let url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}recommended-stock?page=${page}&limit=${limit}`;
        ...
        return {
            ...
            totalPages: Math.ceil((apiResponse.total || 0) / (apiResponse.limit || limit)),
            currentPage: apiResponse.page || 1,
            limit: apiResponse.limit || limit
        };
    } catch (err) {
        console.error("Error fetching recommended stocks:", err);
        return {
            items: [],
            totalItems: 0,
            totalPages: 0,
...
export default function RecommendedStockTablesPage() { 
    ...
    const fetchData = useCallback(async () => {
        try {
            ...
            const data = await fetchRecommendedStocks(pageNum, limit, searchQuery);
            setApiResponse(data);
    ...
    const handleStockCreated = () => {
        // Refresh data when a new stock is created
        fetchData();
        setIsModalOpen(false);
    };

    const handleStockUpdated = () => {
        // Refresh data when a stock is updated
        fetchData();
    };

    const handleStockDeleted = () => {
        // Refresh data when a stock is deleted
        fetchData();
    };

    return (
        <div>
            <PageBreadcrumb pageTitle="Recommended Stocks" />
            <div className="space-y-6">
                <ComponentCard title="Recommended Stock List">
                        ...
                        <form onSubmit={handleSearch} className="flex-1 max-w-md">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    placeholder="Search recommended stocks..."
                    ...
                    <CreateRecommendedStock
                        isOpen={isModalOpen}
                        onClose={() => setIsModalOpen(false)}
                        onSuccess={handleStockCreated}
                    />

                    <RecommendedStockListTable 
                        recommendedStocks={apiResponse.items} 
                        error={error}
