...
export default function CreateCurrency({ isOpen, onClose }: CreateCurrencyProps) {
  ...
  const handleCreateCurrency = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await currencyServiceApi.addCurrency(formData);
    ...
    <div className="fixed inset-0 bg-black-opacity flex items-center justify-center p-4 z-99999">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg dark:bg-gray-900 border border-gray-100 dark:border-gray-800 overflow-hidden">
        <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Add New Currency</h2>
          <button
            onClick={closeModal}
        ...
        <form onSubmit={handleCreateCurrency} className="p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {errors.icon && <p className="text-red-500 text-xs font-medium">{errors.icon}</p>}
          </div>
