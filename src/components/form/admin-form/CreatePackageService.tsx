...
export default function CreatePackage({ isOpen, onClose }: CreatePackageProps) {
  ...
  const [packageData, setPackageData] = useState<PackageServiceData>(DEFAULT_PACKAGE_DATA);
  ...
  const validateForm = () => {
    ...
    const trimmedServiceName = packageData.serviceName.trim();
    const trimmedTitle = packageData.title.trim();
    const trimmedSubtitle = packageData.subtitle.trim();
    const trimmedDescription = packageData.description.trim();
  ...
  const handleCreatePackage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('serviceName', packageData.serviceName.trim());
      formData.append('title', packageData.title.trim());
      formData.append('subtitle', packageData.subtitle.trim());      
      formData.append('description', packageData.description.trim());
      
      // Prepare points array - ensure all points have trimmed text
      const sanitizedFeatures = packageData.points
        .filter(feature => feature.text.trim()) // Filter out empty points
        .map(feature => ({
          text: feature.text.trim()
        }));
      
      // Append points as JSON string (matching your cURL example)
      formData.append('points', JSON.stringify(sanitizedFeatures));
      
      // Append icon file if it exists and we're not skipping file uploads
      if (!skipFileUploads && packageData.icon instanceof File) {
        formData.append('icon', packageData.icon);
      }

      const url = `${process.env.NEXT_PUBLIC_STOCK_API_URL}package-service`;
      ...
      console.log('Package service created successfully:', responseData);
      
      toast.success('Package service created successfully');
      router.refresh();
      closeModal();
    }  catch (error) {
        const errorMsg = error instanceof Error ? error.message : 'Failed to create package service';
        console.error('Error creating package service:', errorMsg, { fullError: error });
  ...
  const addFeature = () => {
    ...
    if (trimmedText) {
      ...
      const isDuplicate = packageData.points.some(
    ...
    <div className="fixed inset-0 bg-black-opacity flex items-center justify-center p-4 z-99999">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl dark:bg-gray-800">
        <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
          <h2 className="text-xl font-semibold dark:text-white">Create New Package Service</h2>
          <button
            onClick={closeModal}
        ...
        <form onSubmit={handleCreatePackage} className="p-4 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            {packageData.icon && !skipFileUploads && (
              <div className="mt-2">
                <Image 
          ...
          <div className="border rounded-lg p-4 dark:border-gray-600">
            ...
            <div className="mt-3 space-y-2">
              {packageData.points.length > 0 ? (
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                    Added Features ({packageData.points.length}):
                  </p>
                  {packageData.points.map((feature, index) => (
          ...
          <div className="flex justify-between gap-3 pt-4">
            ...
            <div className="flex gap-3">
              ...
              >
                {isLoading ? 'Creating...' : 'Create Package Service'}
              </button>
            </div>
          </div>
        </form>
